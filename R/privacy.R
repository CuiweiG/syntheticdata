...
#'
#' @return A `privacy_assessment` object (tibble) with columns:
#'   `metric`, `value`, `risk_level`.
#'
#' @references
#' Snoke J, et al. (2018). General and specific utility measures
#' for synthetic data. \emph{Journal of the Royal Statistical
#' Society A}, 181(3):663--688. \doi{10.1111/rssa.12358}
#'
#' @export
#' @examples
#' set.seed(42)
...
privacy_risk <- function(x, sensitive_cols = NULL) {
  ...
  if (any(num_cols)) {
    ...
    fit <- tryCatch(
      glm(.y ~ ., data = combined, family = binomial()),
    ...
    results <- c(results, list(tibble::tibble(
      ...
      risk_level = ifelse(abs(acc - 0.5) < 0.05, "Low",
                   ifelse(abs(acc - 0.5) < 0.1, "Medium", "High"))
    )))
  }

  ## 3. Attribute disclosure (sensitive column prediction accuracy)
  if (!is.null(sensitive_cols)) {
    for (sc in sensitive_cols) {
      ...
      formula <- as.formula(paste(sc, "~ ."))
      fit <- tryCatch(
        lm(formula, data = syn[, c(sc, pred_cols)]),
        error = function(e) NULL)
      if (!is.null(fit)) {
        pred_vals <- predict(fit,
          newdata = real[, pred_cols, drop = FALSE])
        r2 <- cor(pred_vals, real[[sc]])^2
      } else {
        r2 <- 0
      }
  ...
  structure(out, class = c("privacy_assessment", class(tibble::tibble())))
}
