...
![Three-panel publication-standard comparison of Gaussian copula,
bootstrap, and Laplace-noise synthesis on the UCI Wisconsin Breast
Cancer dataset (N=683, 9 features, 10 seeds per method). Panel a shows
the privacy-utility trade-off as a scatter with 95 percent confidence
ellipses on the plane of distributional fidelity versus NN distance
ratio. Panel b shows multivariate fidelity (Frobenius norm of the
real-minus-synthetic correlation matrix) as a dotplot with median
crossbars. Panel c is a horizontal grouped bar chart of per-feature KS
distances (median and IQR across seeds), variables named from the UCI
codebook. Rendered at 600 DPI via ragg with Okabe-Ito colorblind-safe
palette; vector PDF at
man/figures/privacy_utility_hero.pdf.](reference/figures/privacy_utility_hero.png)
